...
# Fucntion Definition
def calculate_avg_vect(S):
    sum_vect = 0
    for i in S:
        sum_vect += i.T

    return sum_vect / mat_dim
...
# Fucntion Definition
def embedding_feats(list_of_vects, list_of_labels, data_vect, word_limit):
...
if __name__ == "__main__":
    ...
    for mat_dim in mat_dim_list:
        for iva_type in iva_type_list:
            ...
            data_vect = calculate_avg_vect(S)

            train_vectors, train_labels = embedding_feats(x_train, y_train, data_vect, word_limit)
            test_vectors, test_labels = embedding_feats(x_test, y_test, data_vect, word_limit)
    ...
    results_file = 'model_results_avgerage.pkl'
