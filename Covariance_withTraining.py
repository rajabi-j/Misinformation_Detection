...
# Fucntion Definition
def calculate_s(mat_dim, iva_type):
    ...
    S_list = []
    for W_1, val in zip(Ws, data):
        S_i = W_1 @ (val.T)
        S_list.append(S_i)
    return np.array(S_list)

# Fucntion Definition
def calculate_covect(S):
    S_T = np.transpose(S, (2, 1, 0))
    covect = []
    for S_i in S_T:
        comat_i = np.cov(S_i, rowvar=False)
        # Get the indices of the upper triangular part excluding the diagonal
        triu_indices = np.triu_indices_from(comat_i, k=0)
        # Extract the upper triangular part using the indices
        vect = comat_i[triu_indices]

        covect.append(vect)

    return np.array(covect)
...
# Fucntion Definition
def embedding_feats(list_of_vects, list_of_labels, data_vect):
    DIMENSION = int(mat_dim * (mat_dim+1) / 2)
    feats = []
    labels = []
...
if __name__ == "__main__":
    ...
    for mat_dim in mat_dim_list:
        for iva_type in iva_type_list:
            ...
            S = calculate_s(mat_dim, iva_type)

            data_vect = calculate_covect(S)

            train_vectors, train_labels = embedding_feats(x_train, y_train, data_vect)
            test_vectors, test_labels = embedding_feats(x_test, y_test, data_vect)
    ...
    results_file = 'model_results_Covariance.pkl'
